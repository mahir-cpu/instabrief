"""
Quick test — runs the full pipeline for Sumo and Raghav only,
creates canvases WITHOUT the attendee column,
and DMs both to Mahir.

Usage (from repo root, with Railway env):
    railway run python batch/test_sumo_raghav.py
    railway run python batch/test_sumo_raghav.py 2026-04-21   # specific date
"""
...
# Who to test and where to post their canvases
TEST_PEOPLE = ["Sumo", "Raghav"]
CHANNEL_MAP = {
    "Sumo": "C08P96GDG6M",
    "Raghav": "C097KCDC6E6",
}
...
def main():
    ...
    print("\n" + "#" * 60)
    print("INSTABRIEF TEST — Sumo & Raghav only")
    ...
    for person_name in TEST_PEOPLE:
        ...
        try:
            ...
            # Also post to the person's channel
            channel_id = CHANNEL_MAP.get(person_name)
            if channel_id:
                send_rundown_dm(channel_id, person_name, friendly_date, canvas_info["canvas_url"])
                print("  Posted to channel " + channel_id)
