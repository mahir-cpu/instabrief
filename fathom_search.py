...
def generate_relationship_context(company_name, matched_meetings, attendee_names=None, email_context=""):
    if not matched_meetings and not email_context:
        return None

    meetings_context = ""
    for i, m in enumerate(matched_meetings or []):
    ...
    most_recent = str(len(matched_meetings or []))
    ...
    email_section = ""
    if email_context:
        email_section = "\n\nEMAIL THREADS WITH " + company_name.upper() + ":\n" + email_context

    source_description = ""
    if matched_meetings and email_context:
        source_description = "Based on these past meeting transcripts AND recent email threads with " + company_name + ":"
    elif matched_meetings:
        source_description = "Based on these past meeting transcripts with " + company_name + ":"
    else:
        source_description = "Based on these recent email threads with " + company_name + " (no past meetings found):"
...
- In relationship_history, add a FINAL entry with meeting_label "Key Points from Email", meeting_date as the most recent email date, key_highlights summarizing the 2-3 most critical signals from email threads (pricing discussions, explicit asks, objections, commitments, or deadlines), outcome summarizing the overall email relationship status, and next_steps_list with any pending items from emails.
- Use email signals to ENRICH what_theyre_looking_for — if emails reveal what they're asking for, requesting, or expecting, weave that in.
- Use email signals to ENRICH next_steps_detailed — if there are unanswered questions, pending deliverables, or follow-ups mentioned in emails, include them.
- Use email signals to ENRICH objections_detailed — if there is pushback, hesitation, or concerns in emails, capture them.
- Use email signals to ENRICH best_approach_warm — if email tone or recent discussions reveal how to frame the conversation, incorporate that.
- Do NOT create separate email sections. Integrate email insights INTO the existing sections alongside Fathom data."""
